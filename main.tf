...
/*
module "ec2_pullImage" {
  source = "./modules/ec2"
  user_data = local.user_data_docker
}

locals {
  user_data_createimage = <<-EOT
    #!/bin/bash
    # Exit inmmediately if a command fails, also Print each command before running it
    set -ex
  ...
  DOCKER_USER=$(aws ssm get-parameter --name "/docker/username" --with-decryption --query "Parameter.Value" --output text)
  DOCKER_PASS=$(aws ssm get-parameter --name "/docker/password" --with-decryption --query "Parameter.Value" --output text)
